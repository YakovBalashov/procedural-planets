...
float3 ApplyDisplacement(float3 positionWS)
{
...
float3 CalculateNormalWS(float3 positionWS)
{
    Normal normalInfo = (Normal)0;
    normalInfo.Initialize(positionWS, _PlanetCenter, _NormalSampleDistance);
    float3 finalNormal = normalInfo.GetNormal(_NoiseSettings, _NoiseLayerCount, _PlanetRadius);
    return finalNormal;
}
                     ...
                     float3 barycentricCoordinates : SV_DomainLocation)
{
    ...
    output.positionWS = ApplyDisplacement(positionWS);

    if (_NormalCalculationMethod == NORMAL_METHOD_ANALYTIC)
    {
        normalWS = CalculateNormalWS(positionWS);
    }
